  ...
  return (
    <div style={{padding:'1rem 1.25rem'}}>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(180px,1fr))',gap:10,marginBottom:14}}>
        <div className="card" style={{padding:14}}><div style={{fontSize:11,color:'var(--cs-text-muted)'}}>📦 إجمالي السجلات</div><div style={{fontSize:24,fontWeight:800,color:'var(--cs-blue)'}}>{totalRecords}</div></div>
        <div className="card" style={{padding:14}}><div style={{fontSize:11,color:'var(--cs-text-muted)'}}>📥 حركات استلام</div><div style={{fontSize:24,fontWeight:800,color:'#16A34A'}}>{totalIN}</div></div>
        <div className="card" style={{padding:14}}><div style={{fontSize:11,color:'var(--cs-text-muted)'}}>📤 حركات استخدام</div><div style={{fontSize:24,fontWeight:800,color:'#DC2626'}}>{totalOUT}</div></div>
      ...
      <div className="card" style={{padding:0,overflow:'auto'}}>
        <table className="cs-table" style={{margin:0,fontSize:13}}>
          <thead><tr><th>الكود</th><th>التاريخ</th><th>الحركة</th><th>النوع</th><th>المواصفة</th><th>الكمية</th><th>القطع</th><th>الفاقد</th><th>التكلفة</th><th>المشروع</th><th>الفني</th><th>إجراءات</th></tr></thead>
          <tbody>
            {filteredMovements.length===0 ? (
              ...
              return (
                <tr key={m.id} style={{background:m.movement_type==='IN'?'#F0FFF4':'#FEF2F2'}}>
                  ...
                  <td style={{fontSize:11}}>{m.movement_type==='IN' ? `${m.num_pieces||0} ق` : '—'}</td>
