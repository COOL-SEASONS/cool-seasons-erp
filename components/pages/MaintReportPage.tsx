...
export default function MaintReportPage() {
  ...
  const openEdit=(r:any)=>{
    setForm({report_no:r.report_no||'',report_date:r.report_date?.split('T')[0]||'',client_id:r.client_id||'',tech_id:r.tech_id||'',customer:r.customer||'',section:r.section||'',unit_type:r.unit_type||'SPLIT',equipment:r.equipment||'',model:r.model||'',serial_no:r.serial_no||'',complaint:r.complaint||'',problem:r.problem||'',call_time:r.call_time||'',attend_time:r.attend_time||'',done_time:r.done_time||'',parts_used:r.parts_used||'',cost:String(r.cost||0),status:r.status||'Open',notes:r.notes||''})
