...
const newForm = () => ({
  ...
  client_id: '', tech_id: '',
  customer: '', section: '', unit_type: 'SPLIT', equipment: '',
  model: '', serial_no: '', complaint: '', problem: '',
  call_time: '', attend_time: '', done_time: '',
...
export default function MaintReportPage() {
  ...
  const printReport = (r: any) => {
    ...
    <style>body{font-family:Tajawal,Arial,sans-serif;padding:30px;direction:rtl}
    ...
    <div className="page-container">
      <div className="page-header">
        ...
        <div style={{ display:'flex', gap:10 }}>
          <button className="btn-secondary" onClick={() => window.print()}><Printer size={16}/> طباعة</button>
      ...
      <div className="table-container">
        <table className="data-table">
          ...
          <tbody>
            {loading ? (
              <tr><td colSpan={9} style={{ textAlign:'center', padding:40 }}><div className="loading-spinner"/></td></tr>
            ) : filtered.length === 0 ? (
              <tr><td colSpan={9} style={{ textAlign:'center', padding:40, color:'var(--cs-text-muted)' }}>لا توجد تقارير</td></tr>
            ) : filtered.map(r => (
              <tr key={r.id}>
                {/* 9 data cells — matches 9 headers exactly */}
                <td style={{ fontWeight:700, color:'var(--cs-blue)' }}>{r.report_no||'—'}</td>
                <td style={{ fontSize:13, color:'var(--cs-text-muted)' }}>{r.report_date||'—'}</td>
        ...
        <div className="modal-overlay" onClick={() => setViewItem(null)}>
          <div className="modal-box" style={{ maxWidth:500 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:'flex', gap:10, marginTop:16, justifyContent:'flex-end' }}>
              <button className="btn-secondary" onClick={() => printReport(viewItem)}><Printer size={15}/> طباعة</button>
          ...
          <div className="modal-box" style={{ maxWidth:620 }} onClick={e => e.stopPropagation()}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:14, marginTop:8 }}>
              <div>
                ...
                <input className="form-input" value={form.report_no}
                  onChange={e => setForm({...form, report_no: e.target.value})} placeholder="MR-1001"/>
              </div>
              <div>
                <label className="form-label">التاريخ</label>
                <input type="date" className="form-input" value={form.report_date}
              ...
              <div>
                ...
                <select className="form-input" value={form.tech_id}
              ...
              <div>
                ...
                <select className="form-input" value={form.unit_type}
              ...
              <div>
                ...
                <select className="form-input" value={form.problem}
              ...
              <div>
                ...
                <input type="time" className="form-input" value={form.call_time}
                  onChange={e => setForm({...form, call_time: e.target.value})}/>
              </div>
              <div>
                <label className="form-label">وقت الوصول</label>
                <input type="time" className="form-input" value={form.attend_time}
                  onChange={e => setForm({...form, attend_time: e.target.value})}/>
              </div>
              <div>
              ...
              <div style={{ gridColumn:'1/-1' }}>
                ...
                <input className="form-input" value={form.parts_used}
                  onChange={e => setForm({...form, parts_used: e.target.value})} placeholder="فلتر، غاز R410A ..."/>
              </div>
              <div style={{ gridColumn:'1/-1' }}>
                <label className="form-label">الشكوى</label>
                <textarea className="form-input" rows={2} value={form.complaint}
