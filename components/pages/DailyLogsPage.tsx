'use client'
const generateCode_DailyLogsPage = (rows: any[]) => {
  if(!rows || !rows.length) return 'DL-700'
  const nums = rows
    .map((r:any) => r.log_code?.toString().replace('DL-','').replace(/\D/g,''))
    .filter(Boolean).map(Number).filter(n => !isNaN(n))
  if(!nums.length) return 'DL-700'
  return 'DL-' + (Math.max(...nums) + 1)
}
import { useState, useEffect } from 'react'
import { supabase } from '@/lib/supabase'
import { Plus, Search, Edit2, Trash2, X, Save, Printer} from 'lucide-react'

const EMPTY = { log_code:`DL-${700+Math.floor(Date.now()/1000)%9000}` as string, log_date: new Date().toISOString().split('T')[0], project_id:'', tech_id:'', work_done:'', obstacles:'', materials_used:'', work_hours:'', notes:'' }
